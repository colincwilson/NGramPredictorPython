...
class WordPredictor:
    ...
    def __init__(self, lm_filename, vocab_filename):
        ...
        self.char_list = {}
        self.char_list[''] = self.create_char_list_from_vocab(
            '', vocab_filename)
        self.trie_table = {}
    ...
    def add_vocab(self, vocab_id, vocab_filename):
        ...
        if vocab_id in self.trie_table:
            print('Vocabulary with id "' + vocab_id +
                  '" already exists, try a new id.')
            return False
        ...
        self.char_list[vocab_id] = self.create_char_list_from_vocab(
            vocab_id, vocab_filename)
    ...
    def get_words(self,
        ...
        return self._get_words(prefix, '', vocab_id, num_predictions,
                               min_log_prob)
    ...
    def get_words_with_context(self,
                               ...
                               num_predictions=0,
                               min_log_prob=float('inf')):
        return self._get_words(prefix, context, vocab_id, num_predictions,
                               min_log_prob)

    def _get_words(self, prefix, context, vocab_id, num_predictions,
                   min_log_prob):
        (state_in, state_out) = self.get_context_state(context,
                                                       self.language_model,
                                                       vocab_id)
        ...
        for char in self.char_list[vocab_id]:
            ...
            # print(new_prefix)
            words_with_logprob = current_trie.get_words_with_prefix(
                new_prefix, self.language_model, state_in, state_out)

            #update the most probable word
            most_prob_word, most_prob_word_log = self.find_most_probable_word(
            ...
            if num_predictions < 1:
                ...
                suggestion_list.append(
                    likely_words_with_logprob[:num_predictions])
    ...
    def get_most_probable_word(self,
        ...
        words_with_log_prob = current_trie.get_words_with_prefix(
            prefix, self.language_model, state_in, state_out)
        # Update the most probable word
        most_prob_word, most_prob_word_log = self.find_most_probable_word(
            words_with_log_prob, most_prob_word, most_prob_word_log)
